...
<table>
<TR><th width=200pt>Name:</th><TD>$(/usr/local/bin/nvram get contact_name)</TD></TR>
<TR><th width=200pt>E-Mail:</th><TD>$(/usr/local/bin/nvram get contact_email)</TD></TR>
</table>
</fieldset>
<br>
<fieldset class="bubble">
...
<table>
<TR><th width=200pt>Internet-Gateway:</th><TD colspan=7>$(
...
<TR><th width=200pt>Auto-Update:</th><TD colspan=7>$(if [ $(/usr/local/bin/nvram get autoupdate) -eq '1' ]; then printf '<img src="/images/yes.png">'; else printf '<img src="/images/no.gif">'; fi)</TD></TR>
<TR><th>Knoten-IP-Adresse:</th><TD colspan=7>$(ip addr show bmx_prime | awk '/inet/ {print $2}' | sed 's/\/.*//')</TD></TR>
...
<TR><th>Firmware-Version:</th><TD colspan=7>Freifunk Dresden Server Edition - $(cat /etc/freifunk-server-version) (Branch: $(/usr/local/bin/nvram get branch))</TD></TR>
<TR><th>Freier Speicher:</th><TD colspan=7></TD></TR>
<TR><th width=200pt></th><th>Total</th> <th>Used</th> <th>Free</th> <th>Shared</th> <th>Buffered/Cached</th> <th>Available</th><th width=30%>&nbsp;</th></TR>
...
$(
	...
	for s in $services
	do
		printf '<TR><th width=200pt>%s:</th><TD>' "$s"
