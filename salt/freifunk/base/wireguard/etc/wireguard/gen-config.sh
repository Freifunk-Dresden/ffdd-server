...
PostUp = ip route add default dev $vpn table gateway_pool metric $metric ; iptables -w -t nat -A POSTROUTING -o $vpn -j SNAT --to-source $ipv4 ; iptables -w -t mangle -A OUTPUT -p icmp --icmp-type fragmentation-needed -j MARK --set-mark 3333 ; /usr/local/bin/freifunk-gateway-check.sh &
PreDown = ip route del default dev $vpn table gateway_pool metric $metric ; iptables -w -t nat -D POSTROUTING -o $vpn -j SNAT --to-source $ipv4 ; iptables -w -t mangle -D OUTPUT -p icmp --icmp-type fragmentation-needed -j MARK --set-mark 3333; ip addr flush dev $vpn ; ip link delete $vpn ; /usr/local/bin/freifunk-gateway-check.sh &
